...
export default function AdminSubscribers() {
  ...
  useEffect(() => {
    fetch('http://localhost:8787/api/admin/subscribers', { headers: { 'Authorization': `Basic ${authHeader}` } })
  ...
  const confirmDelete = async () => {
    ...
    const res = await fetch(`http://localhost:8787/api/admin/subscribers/${deleteConfirmId}`, { method: 'DELETE', headers: { 'Authorization': `Basic ${authHeader}` } });
  ...
  return (
    <div className="admin-table-section">
      <div className="admin-table-header"><h2>Subscribers</h2></div>
      <table className="admin-table">
        <thead><tr><th>ID</th><th>Email</th><th>Date</th><th>Action</th></tr></thead>
        <tbody>
          {items.map(i => <tr key={i.id}><td>{i.id}</td><td>{i.email}</td><td>{new Date(i.created_at).toLocaleDateString()}</td><td><button className="action-btn" onClick={()=>del(i.id)}><Trash2 size={16} /></button></td></tr>)}
        </tbody>
      </table>
      ...
      {deleteConfirmId && (
        <div className="admin-modal-overlay" onClick={() => setDeleteConfirmId(null)}>
          <div className="admin-modal-content" onClick={e => e.stopPropagation()} style={{maxWidth: '400px', textAlign: 'center'}}>
            ...
            <p style={{marginBottom: '25px', color: '#555'}}>Are you sure you want to delete this subscriber?</p>
